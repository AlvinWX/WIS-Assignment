    ...
    if (!$_err) {
        if(json_encode($photo_resources) != "[]"){
            $stm = $_db->prepare('UPDATE product
                                SET product_name = ?, product_desc = ?, product_price = ?, product_stock = ?, product_cover = ?, product_resources = ?, category_id = ?
                                WHERE product_id = ?');
            $stm->execute([$product_name, $product_desc, $product_price, $product_stock, $product_cover, json_encode($photo_resources), $category_id, $id]);
